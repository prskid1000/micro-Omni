...
def load_model_from_safetensors(model_dir, device="cuda"):
    ...
    # Load main config.json (preferred - Hugging Face format)
    main_config_path = os.path.join(model_dir, "config.json")
    main_config = {}
    if os.path.exists(main_config_path):
        with open(main_config_path, "r") as f:
            main_config = json.load(f)
        print("  ✓ Loaded main config.json")
    
    # Load component configs (optional - fallback for backward compatibility)
    configs_dir = os.path.join(model_dir, "configs")
    if not os.path.exists(configs_dir):
        configs_dir = "configs"  # Fallback to default
    
    # Load Thinker config (prefer main config, fallback to component config)
    thinker_cfg = main_config.copy() if main_config else {}
    ...
    if os.path.exists(thinker_cfg_path):
        component_cfg = json.load(open(thinker_cfg_path))
        thinker_cfg.update(component_cfg)  # Component config overrides main config
    ...
    if os.path.exists(talker_cfg_path):
        ...
        talker_cfg = {
            "d_model": 192, "n_layers": 4, "n_heads": 3,
            "d_ff": 768, "codebooks": 2, "codebook_size": 128,
...
def main():
    ...
    # Get context length from config (prefer main config.json)
    main_config_path = os.path.join(args.model_dir, "config.json")
    if os.path.exists(main_config_path):
        with open(main_config_path, "r") as f:
            main_config = json.load(f)
        ctx_len = main_config.get("ctx_len", 512)
    else:
