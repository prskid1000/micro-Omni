...
from omni.data_utils import ASRDataset, collate_mel_text_fn, analyze_asr_dataset
...
def main(cfg):
    ...
    metrics_logger = MetricsLogger(
        ...
        run_id=build_run_id("train_audio_enc.py", cfg.get("config_path"), save_dir),
        metrics_path=os.path.join("logs", "metrics", "train_audio_enc.jsonl"),
        device=device,
    )
    ...
    while epoch < max_epochs:
        ...
        for batch_idx, batch_data in enumerate(pbar, start=enum_start):
            ...
            if batch_step % accumulation_steps == 0:
                ...
                # Update training monitor (LR spike countdown if active)
            ...
            if batch_step % checkpoint_freq == 0 and batch_step > 0:
                ...
                model_data["monitor"] = monitor.get_state_dict()
            ...
            if step % val_freq == 0 and step > 0:
                with ValidationSkipSamplesContext(train_ds):
                    ...
                    monitor.on_val_end(avg_val_loss, opt, {"model": model, "head": head}, logger)
